...
//  AppStoreJSONApis
//
//  Created by Diego Oruna on 2/28/19.
...
class AppFullscreenController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    @objc fileprivate func handleTap() {
        UIView.animate(withDuration: 2, delay: 0, usingSpringWithDamping: 0.7, initialSpringVelocity: 0.7, options: .curveEaseOut, animations: {
//            self.floatingContainerView.transform = .init(translationX: 0, y: -90)
//            self.floatingContainerView.fillSuperview()
            let appSearch = AppsSearchController()
            appSearch.view.frame = self.floatingContainerView.frame
            self.present(appSearch, animated: true, completion: nil)
        })
    }
